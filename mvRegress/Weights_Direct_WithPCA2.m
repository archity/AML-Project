...
SP_Close_ToBePredicted = xlsObjectComplete(:, 19);

input = xlsObjectComplete(:, 1:18);
output = xlsObjectComplete(:, 19);

coeff = mvregress(input, output);
% Returns the estimated coefficients for a multivariate normal regression 

prediction = input(1:679, 1:18).* coeff';
for i = 1 : 679
