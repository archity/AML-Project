...
SP_Close_ToBePredicted = xlsObjectComplete(:, 20);

input = xlsObjectComplete(:, 1:19);
output = xlsObjectComplete(:, 20);

coeff = mvregress(input, output);
% Returns the estimated coefficients for a multivariate normal regression 

prediction = input(1:679, 1:19).* coeff';
for i = 1 : 679
