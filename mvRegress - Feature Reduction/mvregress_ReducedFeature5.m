...
fileToRead = 'S&Pdata_ReducedFeatures5.xlsx';
...
SP_Close_ToBePredicted = xlsObjectComplete(:, 8);

input = xlsObjectComplete(:, 1:7);
output = xlsObjectComplete(:, 8);

coeff = mvregress(input, output);
% Returns the estimated coefficients for a multivariate normal regression 

prediction = input(1:679, 1:7).* coeff';
for i = 1 : 679
