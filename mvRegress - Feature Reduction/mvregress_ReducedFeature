...
fileToRead = 'S&Pdata_ReducedFeatures.xlsx';
...
SP_Close_ToBePredicted = xlsObjectComplete(:, 17);

input = xlsObjectComplete(:, 1:16);
output = xlsObjectComplete(:, 17);

coeff = mvregress(input, output);
% Returns the estimated coefficients for a multivariate normal regression 

prediction = input(1:679, 1:16).* coeff';
for i = 1 : 679
