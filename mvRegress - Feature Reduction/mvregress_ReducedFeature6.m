...
fileToRead = 'S&Pdata_ReducedFeatures6.xlsx';
...
SP_Close_ToBePredicted = xlsObjectComplete(:, 7);

input = xlsObjectComplete(:, 1:6);
output = xlsObjectComplete(:, 7);

coeff = mvregress(input, output);
% Returns the estimated coefficients for a multivariate normal regression 

prediction = input(1:679, 1:6).* coeff';
for i = 1 : 679
